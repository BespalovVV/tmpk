...
@router.post("/reset-forget-password", response_model=SuccessMessage)
async def resetPassword(data: ResetForegetPassword, db: AsyncSession = Depends(get_session)):
    return await NotificationService.reset_forget_password(data, db)

@router.post("/reset-password", response_model=SuccessMessage)
async def resetPassword(data: ResetPassword, request: Request, db: AsyncSession = Depends(get_session)):
    return await NotificationService.reset_password(data, db, request)

@router.post("/send-confirmation-email", response_model=SuccessMessage)
async def send_confirmation_email_route(data: UserNotif, db: AsyncSession = Depends(get_session)):
...
async def confirm_email_route(token: str, db: AsyncSession = Depends(get_session)):
